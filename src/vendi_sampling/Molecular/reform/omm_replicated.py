...
class OMMTReplicas_replicated(OMMTReplicas):
    ...
    def get_rep_state(self, getPositions=False, getVelocities=False, getForces=False, getEnergy=False, groups=None) -> omm.State:
        if groups is not None:
            return self._contexts[0].getState(getPositions=getPositions, getVelocities=getVelocities,
                                          getForces=getForces, getEnergy=getEnergy, groups=groups)
        return self._contexts[0].getState(getPositions=getPositions, getVelocities=getVelocities,
