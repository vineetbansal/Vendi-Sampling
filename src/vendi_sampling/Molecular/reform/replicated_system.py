...
class ReplicatedSystem():
    ...
    def replicate_system(base_system: System, n_replicas: int, enable_energies=False):
        ...
        # properties
        #print('Periodic', base_system.getDefaultPeriodicBoxVectors())
        ...
        for force in base_system.getForces():
            ...
            #if forcename not in used_forces:
            #    used_forces.append(forcename)
            #else: 
            #    continue
            #print(forcename)
    ...
    def _replicate_NonbondedForce(force, n_particles, n_replicas, enable_energies):
        ...
        print(nonbonded_method)
        print(NonbondedForce.NoCutoff)
                ...
                for p2 in range(p1+1, force.getNumParticles()):
                    if (p1,p2) in exceptions:
                        ...
                        if (
                            ...
                            replicated_force.addBond(p1 + j*n_particles, p2 + j*n_particles, [qiqj, epsilon, sigma])
            '''
            current_replica_atoms = [i for i in range(j*n_particles, (j+1)*n_particles)]
            for k in range(n_replicas): # loop over replicas
                if k==j: # if current replica, skip, no additional exclusions
                    continue
                for i in range(force.getNumParticles()): # loop over particles
                    for n in current_replica_atoms: # loop over atoms in current replica
                        if (i + k*n_particles) not in current_replica_atoms: # check of particle is not in current replica
                            replicated_force.addException(i + k*n_particles, n, chargeProd=0, epsilon=0, sigma=0.5) # if not, set custom bondforce to 0 (chargeprod=sigma0)
            '''
            if enable_energies:
                replicated_force.setForceGroup(j)
                replicated_forces.append(replicated_force)
    ...
    def _replicate_CustomGBForce(force, n_particles, n_replicas, enable_energies):
        ...
        #for _ in range(n_replicas):
            #print(force.getNumEnergyTerms())
        replicated_force = CustomGBForce()
        print('Using Periodic Boundary', force.usesPeriodicBoundaryConditions())
        print('Cutoff Dist (nm) ', force.getCutoffDistance())
        replicated_force.setCutoffDistance(force.getCutoffDistance())
        #replicated_force.setUsesPeriodicBoundaryConditions(force.usesPeriodicBoundaryConditions())
        #print(
        for i in range(force.getNumPerParticleParameters()):
            name = force.getPerParticleParameterName(i)
            #print(name)
            replicated_force.addPerParticleParameter(name)
        for i in range(force.getNumGlobalParameters()):
            name = force.getGlobalParameterName(i)
            #print(name)
            value = force.getGlobalParameterDefaultValue(i)
            replicated_force.addGlobalParameter(name, value)
        for i in range(force.getNumComputedValues()):
            name,expression,type = force.getComputedValueParameters(i)
            #print(expression)
            replicated_force.addComputedValue(name, expression, type)
        for i in range(force.getNumEnergyTerms()):
            expression, type = force.getEnergyTermParameters(i)
            #print(expression)
            replicated_force.addEnergyTerm(expression, type)
        for i in range(force.getNumExclusions()):
            p1, p2 = force.getExclusionParticles(i)
            replicated_force.addExclusion(p1, p2)
        for i in range(force.getNumEnergyParameterDerivatives()):
            name = force.getEnergyParameterDerivativeName(i)
            replicated_force.addEnergyParameterDerivative(name)
        for i in range(force.getNumEnergyParameterDerivatives()):      
            name = force.getEnergyParameterDerivativeName(i)
            value = force.getEnergyParameterDerivative(i)
            replicated_force.setEnergyParameterDerivative(i, value)
        ...
        for j in range(n_replicas):
            for i in range(force.getNumParticles()):    
                ...
                #print(parameters)
                if j>-1:
                    replicated_force.addParticle(parameters)
                else:
                    replicated_force.addParticle((0,parameters[1],parameters[2]))
        

        #for j in range(n_replicas): 
        #    for i in range(force.getNumParticles()):
        #        if j!=_: # put parameters to zero if Particle does not belong to current replica
        #            parameters = (1, 1, 1)
        #        replicated_force.setParticleParameters(i + j*n_particles, parameters)
            #replica_range = [j*n_particles, (j+1)*n_particles]

        #current_replica_atoms = [i for i in range(_*n_particles, (_+1)*n_particles)]

        for x in range(n_replicas):
            current_replica_atoms = [i for i in range(x*n_particles, (x+1)*n_particles)]
            for j in range(x+1, n_replicas): # loop over replicas
                if j==x: # if current replica, skip, no additional exclusions
                    continue
                for i in range(force.getNumParticles()): # loop over particles
                    for n in current_replica_atoms: # loop over atoms in current replica
                        if (i + j*n_particles) not in current_replica_atoms: # check of particle is not in current replica
                            replicated_force.addExclusion(i + j*n_particles, n) # if not add exclusion
        print(replicated_force.getNumExclusions())       

        #replicated_forces.append(replicated_force)

        #for i in range(force.getNumExceptions()):
        #    p1, p2, q, sigma, epsilon = force.getExceptionParameters(i)
        #    replicated_force.setExceptionParameters(i, p1 + j*n_particles, p2 + j*n_particles, q, sigma, epsilon)
        if enable_energies:
